...
module.exports = class Sessions {
    ...
    static async start(sessionName, options = []) {
	  ...
	  var token_path = `tokens/${sessionName}.data.json`
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                try {
                    ...
                    return await client.sendText(req.query.to + '@c.us', req.query.msg);
                });
    ...
    static async sendText(req) {
        ...
        if (session) {
		    ...
		    console.log(req.body);
                    return await client.sendText(req.body.number + '@c.us', req.body.text);
                });
              return { result: "success" }
                } catch (error) {
                        console.error(error);
    ...
    static async sendToGroup(req) {
		...
		try {
                await session.client.then(async client => {
                    console.log('#### send msg =', params);
                    return await client.sendText(params.groupID + "@g.us", params.text);
                });
                return { result: "success" }
		} catch (error) {
			console.error(error);
    ...
    static async getGroupMembers(req) {
        var params = {
            sessionName: req.body.sessionName,
            groupID: req.body.groupID,
        }
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var result =  await session.client.then(async client => {
                    return await client.getGroupMembers(params.groupID + '@g.us');
                });
                return { result }
            } else {
                return { result: "error", message: session.state };
   ...
   static async sendImage(sessionName, number, image) {
        var session = Sessions.getSession(sessionName);
        if (session) {
            if (session.state == "CONNECTED") {
                var resultSendImage = await session.client.then(async (client) => {
                    return await client.sendImage(number + '@c.us', image);
                }); //client.then(
                return {
                    result: "success"
                };
    ...
    static async sendFileB64(sessionName, number, base64Data, fileName, caption) {
        var session = Sessions.getSession(sessionName);
        if (session) {
            if (session.state == "CONNECTED") {
    ...
    static async sendFile(sessionName, number, file) {
        var session = Sessions.getSession(sessionName);
        if (session) {
            if (session.state == "CONNECTED") {
                var resultSendFile = await session.client.then(async (client) => {
                    return await client.sendFile(number + '@c.us', file , file , "Estado da Arte!");
                }); //client.then(
                return { result: "success" };
            } else {
                return { result: "error", message: session.state };
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var resultSendVoice = await session.client.then(async (client) => {
                    return await client.sendVoice(number + '@c.us', voice);
                }); //client.then(
                return {
                    result: "success"
                };
    ...
    static async getAllUnreadMessages(sessionName) {
       ...
       static async getAllMessages(sessionName, number) {
        var session = Sessions.getSession(sessionName);
        var numero = number;
	var includeMe = true;
        if (session) {
            if (session.state == "CONNECTED") {
                var resultGetAllMessages = await session.client.then(async (client) => {
                     await client.getAllMessagesInChat(number + "@c.us", includeMe);
                    return await client.loadAndGetAllMessagesInChat(number + "@c.us", includeMe);
                });
                return {
                    result: resultGetAllMessages
                };
            } else {
                return {
                    result: "error",
	...
	let session = Sessions.getSession(sessionName);
 	if (session) {
 		if (session.state == "CONNECTED") {
			...
			let grupos = [];
			let chats = await session.client.then(async (client) => {
				return await client.getAllChatsNewMsg();
			});
			console.log(chats);	
...
} // recebe a sessão e retorna os grupos em json
